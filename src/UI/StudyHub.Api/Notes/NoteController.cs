...
public sealed class NoteController(INoteOrchestrator noteOrchestrator) : ControllerBase
{
    ...
    public Task<IReadOnlyList<NoteDto>> GetByCourseIdAsync(
        Guid courseId,
        CancellationToken cancellationToken
    ) => noteOrchestrator.GetByCourseIdAsync(courseId, cancellationToken);

    [HttpGet("by-semester/{semesterId:guid}")]
    public Task<IReadOnlyList<NoteDto>> GetBySemesterIdAsync(
        Guid semesterId,
        CancellationToken cancellationToken
    ...
    public Task<IReadOnlyList<NoteBacklinkDto>> GetBacklinksAsync(
        Guid id,
        CancellationToken cancellationToken
    ) => noteOrchestrator.GetBacklinksAsync(id, cancellationToken);

    [HttpPost]
    public Task<NoteDto> CreateAsync(
    ...
    [HttpPut("{id:guid}")]
    public Task<NoteDto> UpdateAsync(
        Guid id,
    ...
    public Task<NoteDto> AttachDocumentAsync(
        Guid id,
        Guid documentId,
        CancellationToken cancellationToken
    ...
    public Task<NoteDto> DetachDocumentAsync(
        Guid id,
        Guid documentId,
        CancellationToken cancellationToken
